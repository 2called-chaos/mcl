  ...
  class HSnap2date < Handler
    ...
    def setup_parsers
      register_command :snap2date do |handler, player, command, target, optparse|
        ...
        case args[0]
        when "status"
          handler.tellm(player, {text: "watched versions", color: "gold"}, {text: handler.watched_versions.join(" "), color: "reset"})
          handler.tellm(player, {text: "watcher enabled?", color: "gold"}, {text: handler.cron.to_s, color: "reset"})
        when "check"
          # @todo CHECK STUFF
        when "watch"
          if args[1]
            handler.watch_version args[1].downcase
            handler.tellm(player, {text: "Watching version #{args[1].downcase}...", color: "reset"})
          else
            handler.tellm(player, {text: "Define a version to watch!", color: "red"})
          end
        when "unwatch"
          if args[1]
            handler.unwatch_version args[1].downcase
            handler.tellm(player, {text: "Stop watching version #{args[1].downcase}...", color: "reset"})
          else
