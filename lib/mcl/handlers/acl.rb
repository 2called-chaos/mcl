module Mcl
  ...
  class HMMclACL < Handler
    def setup
      ...
      register_deop(:admin)
      register_acl(:admin)
      register_uadmin(:guest)
    end

    def register_op acl_level
      register_command :op, desc: "ops you or a target (no selectors)", acl: acl_level do |player, args|
        $mcl.server.invoke "/op #{args.first || player}"
      end
    end

    def register_deop acl_level
      register_command :deop, desc: "deops you or a target (no selectors)", acl: acl_level do |player, args|
        $mcl.server.invoke "/deop #{args.first || player}"
      end
    end

    def register_acl acl_level
      register_command :acl, desc: "manage ACL rules/cache", acl: acl_level do |player, args|
        case args[0]
    ...
    def register_uadmin acl_level
      register_command :uadmin, desc: "Gives yourself ACL root permissions if listed as uadmin in config", acl: acl_level do |player, args|
